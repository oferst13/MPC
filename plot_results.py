...
# ax = results_df.plot(x='Total_mm', y='Water_reduction-perfect', kind='scatter', marker='o', color='xkcd:dark sky blue', label='Perfect', s=40)
boxplot = True
histo = False
if boxplot:
    ...
    results_df.plot(x='Total_mm', y='Water_reduction-plusMin', kind='scatter', marker=(5,1), ax=ax,
                    color='xkcd:bright orange', label='Depth', s=35)
    results_df.plot(x='Total_mm', y='Water_reduction-swap', kind='scatter', marker='+',ax=ax, color='xkcd:lightish green', label='Depth & swap',s=30)
    ax.legend(title='Forecast type', fontsize=11)
# plt.xlabel('Max EDf5 ' + r'$(\frac{mm}{hr})$', fontsize=12)
#plt.xlabel('Event depth (mm)', fontsize=12)

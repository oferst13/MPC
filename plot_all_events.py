...
def plot_compare(outflow1, outflow2, units, outflow3=None, legend1='Baseline', legend2='Controlled',
    ...
    axs[1].plot(cfg.hours[np.nonzero(cfg.hours <= plot_hours)],
                outflow1[0:len(cfg.hours[np.nonzero(cfg.hours <= plot_hours)])], 'xkcd:dark sky blue',
                label=legend1, linewidth=4.1, linestyle='-')
    axs[1].plot(cfg.hours[np.nonzero(cfg.hours <= plot_hours)],
                outflow2[0:len(cfg.hours[np.nonzero(cfg.hours <= plot_hours)])], 'r',
                label=legend2, linewidth=3.3, linestyle='-.')
    if outflow3.any():
        axs[1].plot(cfg.hours[np.nonzero(cfg.hours <= plot_hours)],
                    outflow3[0:len(cfg.hours[np.nonzero(cfg.hours <= plot_hours)])],
                    'xkcd:goldenrod', label=legend3, linewidth=2.6, linestyle='--')
    if units == 'CMS':
...
for file in cfg.files:
    ...
    optim_flow = np.zeros((len(suffix_list), len(baseline.swmm_flow)))
    for idx, suff in enumerate(suffix_list):
        Pipe.reset_pipe_all(sim_len, 'factory')
        Tank.reset_all(sim_len, 'factory')
        Tank.set_inflow_forecast_all(act_rain)
